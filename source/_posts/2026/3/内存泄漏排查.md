...
### 补充内容
修改 RecipeFilterViewModel.cs：
提供对外的清理接口，利用子类权限强行注销父类注册的事件。
``` csharp
public void Cleanup()
{
    // 切断最致命的静态引用（拔掉铁钩）
    Cimetrix.CimControlFramework.WPFGuiCommon.Framework.Localization.LocalizationManager.CultureChanged -= this.CultureChanged;
}
```

目前的改法
``` diff
 		public RecipeFilterViewModel(CCFRecipe.RecipeType recipeType, ICommand recipeRowSelectCommand, ICommand recipeDoubleClickCommand)
 		{
 			this.recipeType = recipeType;
-			CreateRecipeFilter(recipeRowSelectCommand, recipeDoubleClickCommand);
+			CreateRecipeFilter(recipeRowSelectCommand, recipeDoubleClickCommand);
+			LocalizationManager.CultureChanged -= CultureChanged;
 		}
 
 		public RecipeFilterViewModel(CCFRecipe.RecipeType recipeType, ICommand recipeRowSelectCommand, ICommand recipeDoubleClickCommand, bool showEmptyItem, string[] filterRecipeTypes)
@@ -64,7 +66,8 @@ namespace BeanUI.Controls
 			this.recipeType = recipeType;
 			CreateRecipeFilter(recipeRowSelectCommand, recipeDoubleClickCommand);
 			this.showEmptyItem = showEmptyItem;
-			ListRecipe(filterRecipeTypes);
+			ListRecipe(filterRecipeTypes);
+			LocalizationManager.CultureChanged -= CultureChanged;
 		}
 
 		private void CreateRecipeFilter(ICommand recipeRowSelectCommand, ICommand recipeDoubleClickCommand)
